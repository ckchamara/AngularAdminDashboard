...
export class LottoEnterComponent implements OnInit {
  ...
  constructor(agc: RestserviceService, private lotteryComponentsService: LotteryComponentsService, public dialog: MatDialog) {
  ...
  ButtonClickcreateIndex(): void {
    this.addNewWinningNumber(this.lotteryOptions);
    console.log(this.drawNo);
    this.getResultValues(this.lotteryOptions);
    this.openSetResultDialog();
  }
  ...
  addNewWinningNumber(lotteryOptions): void {
    const reqBody = {
      name: lotteryOptions.name,
      id: lotteryOptions.id,
      drawNo: this.drawNo,
    ...
    this.httpclientservice.addHero('http://localhost:9090/db/createIndex', reqBody)
      .subscribe(configResponce => {
        this.lotteryComponentsService.setResultConfig(configResponce);
